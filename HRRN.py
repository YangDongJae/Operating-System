...
class HRRN(SchedulingAlgorithm):
    ...
    def schedule(self):                     #스케줄링 함수
        ...
        while self.process_queue or self.ready_queue or processor0.current_process or processor1.current_process or processor2.current_process or processor3.current_process:
            ...
            if self.ready_queue:
                self.ready_queue=sorted(self.ready_queue, key=lambda process:((process.waiting_time+process.burst_time)/process.burst_time), reverse=True)            
...
class Main:
    ...
    def run_scheduler(self):
        for process in self.processes:
            self.hrrn_algorithm.add_process(process)
        self.hrrn_algorithm.schedule()

    def print_result(self):
        return self.hrrn_algorithm.print_results()
